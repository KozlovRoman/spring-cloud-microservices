...
public class DepositService {
    ...
    private final RabbitTemplate rabbitTemplate;            // Service will send a message to Rabbit
    ...
    public DepositResponseDTO deposit(Long accountId, Long billId, BigDecimal amount) {
        ...
        if (billId != null) {
            ...
            AccountResponseDTO accountResponseDTO = accountServiceClient.getAccountById(billResponseDTO
                    .getAccountId());                                   // For knowing email
                                                                        // (from Account)
        ...
        depositRepository.save(new Deposit(amount, defaultBill.getBillId(), OffsetDateTime.now(),
                account.getEmail()));                                   // Entity saving
    ...
    private DepositResponseDTO createResponse(BigDecimal amount, AccountResponseDTO accountResponseDTO) {
        ...
        // For response to controllers and send this depositResponseDTO to RabbitMQ. Now we can
        ...
        } catch (JsonProcessingException e) {                       // Checked exception from jackson
            e.printStackTrace();
    ...
    private BillRequestDTO createBillRequest(BigDecimal amount, BillResponseDTO billResponseDTO) {
        BillRequestDTO billRequestDTO = new BillRequestDTO();           // DTO will be sent for Bill updating
    ...
    private BillResponseDTO getDefaultBill(Long accountId) {
        return billServiceClient.getBillsByAccountId(accountId).stream()
                ...
                .findAny()
                .orElseThrow(() -> new DepositServiceException("Unable to find default " +
                        "bill for account: " + accountId));
    }
}
